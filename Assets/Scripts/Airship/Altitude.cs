...
public class Altitude : MonoBehaviour, IInteractable
{
    ...
    void Update()
    {
        ...
        float shipHeight = Mathf.InverseLerp(-actualHeightRange, actualHeightRange, Airship.Transform.position.y);
        shipGauge.localPosition = shipGauge.localPosition.WithY(Remap.Float(shipHeight, 0, 1, -gaugeRange, gaugeRange));
